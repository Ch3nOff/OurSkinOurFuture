...
export default function ManualCrop({ image, onConfirm, onCancel }) {
  ...
  const zoomIn = () => {
    setCrop((prev) => {
      const newSize = Math.min(1, prev.size + 0.1);
  ...
  const zoomOut = () => {
    setCrop((prev) => {
      const newSize = Math.max(0.3, prev.size - 0.1);
      const maxX = 1 - newSize;
      const maxY = 1 - newSize;
      const newX = Math.min(maxX, Math.max(0, prev.x - (newSize - prev.size) / 2));
      const newY = Math.min(maxY, Math.max(0, prev.y - (newSize - prev.size) / 2));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div
          ref={containerRef}
          className="relative rounded-2xl overflow-hidden border border-border bg-paper select-none touch-none"
        ...
        >
          ...
          />
          <div
            className="absolute border-2 border-gold bg-transparent cursor-move"
            style={{
              left: `${crop.x * 100}%`,
              top: `${crop.y * 100}%`,
              width: `${crop.size * 100}%`,
