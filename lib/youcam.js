...
async function startTask(task, body) {
  ...
  console.log(`YouCam ${task} request:`, JSON.stringify(body).slice(0, 400), `status:`, res.status, text.slice(0, 200));
  if (!res.ok) {
    throw new Error(`YouCam task start failed (${res.status}): ${text}`);
  }
 ...
 * Tries multiple body shapes because the API validator is strict.
 */
export async function simulateWithYouCamFromUrl(imageUrl, intensity = 0.6) {
  ...
  const bodies = [
    // 1. Exact skin-analysis shape (known working) with simulation params.
    () =>
      startTask("skin-simulation", {
        src_file_url: imageUrl,
        dst_actions: ["acne", "wrinkle", "redness", "dark_circle_v2", "pore", "texture", "age_spot", "moisture", "tear_trough", "droopy_upper_eyelid", "droopy_lower_eyelid", "eye_bag", "firmness", "oiliness", "radiance", "skin_type"],
        miniserver_args: { enable_mask_overlay: true },
        format: "json",
        pf_camera_kit: false,
      }),
    // 2. Flat top-level with concern values.
    () =>
      startTask("skin-simulation", {
        src_file_url: imageUrl,
        ...simParams,
      }),
    // 3. Nested under `body` wrapper.
    () =>
      startTask("skin-simulation", {
        src_file_url: imageUrl,
        body: {
    ...
    // 4. Analysis-style wrapper with flat values.
    () =>
      startTask("skin-simulation", {
        src_file_url: imageUrl,
        dst_actions: simParams,
        miniserver_args: { enable_mask_overlay: true },
        format: "json",
        pf_camera_kit: false,
      }),
    // 5. With file_id (upload first).
    async () => {
      const { fileId } = await uploadImageFromUrl(imageUrl);
      return startTask("skin-simulation", {
        src_file_id: fileId,
        dst_actions: ["acne", "wrinkle", "redness", "dark_circle_v2", "pore", "texture", "age_spot", "moisture", "tear_trough", "droopy_upper_eyelid", "droopy_lower_eyelid", "eye_bag", "firmness", "oiliness", "radiance", "skin_type"],
        miniserver_args: { enable_mask_overlay: true },
        format: "json",
  ...
  const lastErr = bodies.reduce((prev, bodyFn) => {
    return prev.catch((err) => err);
  }, Promise.reject(new Error("All simulation attempts failed.")));

  for (const bodyFn of bodies) {
    try {
      const taskId = await bodyFn();
      return await pollSimulation(taskId);
    } catch (err) {
      console.warn("Sim attempt failed:", err.message);
    }
  }
  throw lastErr;
}
