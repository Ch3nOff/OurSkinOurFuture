 ...
 * Uses the exact parameter set from the API docs example:
 *   src_file_url + acne, dark_circle, eye_bags, oiliness, pores,
 *   radiance, redness, spots, texture, wrinkle
 */
export async function simulateWithYouCamFromUrl(imageUrl, intensity = 0.6) {
  ...
  // Try with src_file_url (same shape as the working API docs example).
  try {
  ...
  // Fallback: upload via File API and use src_file_id.
  try {
