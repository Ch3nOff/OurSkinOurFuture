...
async function uploadImage(buffer, contentType, task) {
  ...
  // S3 pre-signed URLs require the exact headers that were signed.
  // Build headers carefully: include all signed headers, set Content-Type,
  // and let fetch compute Content-Length from the body automatically.
  const putHeaders = { ...signedHeaders };
  if (!putHeaders["Content-Type"]) {
    putHeaders["Content-Type"] = contentType || "image/jpeg";
  }
  // Remove content-length if present — fetch will set it correctly.
  ...
  const putRes = await fetch(uploadUrl, {
    ...
    headers: putHeaders,
    body: blob,
    // @ts-ignore - Next.js fetch supports duplex in newer versions
    duplex: "half",
  });
  if (!putRes.ok) {
...
export async function analyzeWithYouCam(buffer, contentType) {
  const { fileId, uploadUrl } = await uploadImage(buffer, contentType, "skin-analysis");
  const taskId = await startTask("skin-analysis", {
    src_file_url: uploadUrl,
    dst_actions: [
      "acne",
      "wrinkle",
      "redness",
  ...
  // Derive per-zone scores from concern averages so the zone map renders
  // with real-ish values even though YouCam doesn't return per-zone numbers.
...
export async function simulateWithYouCam(buffer, contentType, intensity = 0.6) {
  const { uploadUrl } = await uploadImage(buffer, contentType, "skin-simulation");
