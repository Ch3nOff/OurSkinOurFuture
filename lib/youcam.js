 ...
 * Tries src_file_url first; if the API rejects it, falls back to
 * uploading via the File API and using src_file_id.
 */
export async function simulateWithYouCamFromUrl(imageUrl, intensity = 0.6) {
  ...
  // Try with src_file_url first (same pattern as analysis).
  try {
    ...
    console.warn("Simulation with src_file_url failed, trying src_file_id:", err.message);
  }
