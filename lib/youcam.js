 ...
 * The simulation endpoint expects all parameters inside a `body` object,
 * including the source URL and all concern intensity values.
 */
export async function simulateWithYouCamFromUrl(imageUrl, intensity = 0.6) {
  // Try with body wrapper (per simulation docs schema).
  try {
    ...
    console.warn("Simulation with body wrapper failed, trying fallback:", err.message);
  }

  // Fallback: upload via File API and use src_file_id with body wrapper.
