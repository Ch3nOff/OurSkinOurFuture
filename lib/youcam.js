...
export async function uploadImage(buffer, contentType, task) {
...
export async function startTask(task, body) {
...
export async function pollTask(task, taskId, timeoutMs = 90000) {
...
export async function analyzeWithYouCamFromUrl(imageUrl) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastErr = err;
      console.warn(`YouCam skin analysis attempt ${attempt} failed:`, err.message);
      if (attempt < 3) {
        await new Promise((r) => setTimeout(r, 2000 * attempt));
      }
    }
  }
  throw lastErr || new Error("YouCam skin analysis failed after retries.");
}
...
export async function simulateWithYouCamFromUrl(imageUrl, intensity = 0.6) {
  ...
  const bodies = [
    ...
    async () => {
      if (!imageUrl.startsWith("data:")) throw new Error("Not a data URL");
