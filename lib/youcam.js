...
export async function simulateWithYouCamFromUrl(imageUrl, intensity = 0.6) {
  ...
  // 1. Exact shape from user's working example: flat top-level.
  try {
    ...
    console.warn("Sim attempt 1 (flat url+params) failed:", err.message);
  }

  // 2. Same but with analysis-style wrapper fields (miniserver_args, etc).
  try {
    const taskId = await startTask("skin-simulation", {
      src_file_url: imageUrl,
    ...
    console.warn("Sim attempt 2 (wrapped url+dst_actions) failed:", err.message);
  }

  // 3. src_file_id + flat params (upload first).
  try {
    ...
    console.warn("Sim attempt 3 (flat id+params) failed:", err.message);
  }

  // 4. src_file_id + dst_actions wrapper.
  try {
    const { fileId } = await uploadImageFromUrl(imageUrl);
    const taskId = await startTask("skin-simulation", {
      src_file_id: fileId,
    ...
    console.warn("Sim attempt 4 (wrapped id+dst_actions) failed:", err.message);
  }

  throw new Error("All simulation attempts failed.");
}
