...
export function compareScans(current, previous) {
  const deltas = {};
  const currentScores = current.concern_scores || {};
  const previousScores = previous.concern_scores || {};
  const currentTime = new Date(current.created_at || Date.now()).getTime();
  const previousTime = new Date(previous.created_at || Date.now()).getTime();
  const allKeys = new Set([...Object.keys(currentScores), ...Object.keys(previousScores)]);
  
  allKeys.forEach((key) => {
    const currentVal = currentScores[key];
    const previousVal = previousScores[key];
    if (typeof currentVal !== "number" || typeof previousVal !== "number") return;
    deltas[key] = {
      current: currentVal,
      previous: previousVal,
      delta: previousVal - currentVal,
      label: typeof CONCERN_LABELS[key] === "string" ? CONCERN_LABELS[key] : CONCERN_LABELS[key]?.label || key,
    };
  });
  ...
  const overallPrevious = entries.length
    ...
    : 0;
  const daysBetween = Number.isFinite(currentTime - previousTime)
    ? Math.round((currentTime - previousTime) / (1000 * 60 * 60 * 24))
    : 0;
